...
## Europe first doses in ALL category

# eu_first_dose_all <- data%>%
#   filter(target_group=="ALL")%>%
#   group_by(target_group,region)%>%
#   summarize(total=sum(first_dose))
# 
# eu_first_dose_all_perc <- data%>%
#   filter(target_group=="ALL")%>%
#   distinct(region,.keep_all=T)%>%
#   select(target_group,region,denominator)%>%
#   left_join(eu_first_dose_all,by=c("target_group","region"))%>%
#   mutate(percentage=total/denominator * 100)%>%
#   mutate(percentage=round(percentage,2))%>%
...
europe_first_dose_final%>%
  ...
  geom_text(aes(label = Adults_over_18 * 100, y = Adults_over_18 + 0.03), size = 4)+
...
# ###
# # Denominators (population) by target group from vaccines dataset
# 
# data%>%
#   filter(region == "EL")%>%
#   distinct(target_group, .keep_all = T)%>%
...
# first_dose_germany <- data%>%
#   filter(region == "DE")%>%
#   group_by(target_group,region)%>%
#   summarize(total_first_dose=sum(first_dose))%>%
#   arrange(desc(total_first_dose))%>%
#   pull(total_first_dose)
# 
# first_dose_greece <- data%>%
#   filter(region == "EL")%>%
#   group_by(target_group, region)%>%
#   summarize(total_first_dose = sum(first_dose))%>%
#   ungroup()%>%
#   arrange(desc(total_first_dose))%>%
#   filter(!(target_group %in% c("ALL", "1_Age<60","1_Age60+","Age<18", "HCW", "LTCF")))%>%
#   summarize(total = sum(total_first_dose))%>%
#   pull(total)
# 
# first_dose_portugal <- data%>%
#   filter(region == "PT")%>%
#   group_by(target_group, region)%>%
#   summarize(total_first_dose = sum(first_dose))%>%
#   ungroup()%>%
...
#   pull()
# 
# # Population of countries from cases dataset
# pop_greece <- data_for_population%>%
#   filter(countriesAndTerritories == "Greece")%>%
#   distinct(popData2020)%>%
#   pull()
# 
# pop_germany <- data_for_population%>%
#   filter(countriesAndTerritories == "Germany")%>%
#   distinct(popData2020)%>%
#   pull()
# 
# pop_portugal <- data_for_population%>%
#   filter(countriesAndTerritories == "Portugal")%>%
#   distinct(popData2020)%>%
#   pull()
# 
# pop_ireland <- data_for_population%>%
...
# ## Germany
# 
# first_dose_germany / pop_germany * 100
# ## Greece
# first_dose_greece / pop_greece * 100
# ## Portugal
# first_dose_portugal / pop_portugal * 100
# ## Ireland
# first_dose_ireland / pop_ireland * 100
# 
# # Writing function for the above
# 
#   data%>%
#   #filter(region == "EL")%>%
#   group_by(target_group, region)%>%
